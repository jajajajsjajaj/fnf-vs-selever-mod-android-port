...
class PlayState extends MusicBeatState
{
	...
	public static var firstAttempt:Bool = true; //
	...
	public var dialogue:Array<String> = ['L|dad|blah blah blah', 'R|bf|coolswag'];
	...
	override public function create()
	{
		...
		//Ever heard of try-catch, you imbecile? ¬_¬"
		try { //Cargar diálogo de inicio, de existir alguno
			dialogue = CoolUtil.coolTextFile(Paths.txt('data/' + songLowercase + '/dialogue'));
		} catch (ex:Exception) {
			dialogue = null;
		}

		try { //Cargar diálogo de salida, de existir alguno
			dialogueEnd = CoolUtil.coolTextFile(Paths.txt('data/' + songLowercase + '/dialogue-end'));
		} catch (ex:Exception) {
			dialogueEnd = null;
		}
				...
				case 'Iglesia':
					{
						...
						defaultCamZoom = 0.7;
						curStage = 'Iglesia';
						var bg:FlxSprite = new FlxSprite(-55, -305).loadGraphic(Paths.image('Selever/Base'));
						...
						*/
						...
						defaultCamZoom = 0.7;
						curStage = 'Iglesia';
						var stageFront:FlxSprite = new FlxSprite(-550, -930).loadGraphic(Paths.image('Selever/Base'));
						...
						if (songLowercase == 'demonila'){


						}

						if (songLowercase == 'casanova'){


						}

						if (songLowercase == 'attack'){


						}
						
						/*
						var stageCurtains:FlxSprite = new FlxSprite(160, -140).loadGraphic(Paths.image('Selever/Columnas'));
						stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 1.0));
						stageCurtains.setGraphicSize(Std.int(stageCurtains.height * 1.0));
		...
		*/
		...
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				...
				/*default:
					if (doof == null) startCountdown(); //*/
			}
			if (firstAttempt && doof != null) { //Mostrar diálogos en cualquier canción que las tenga
				switch (StringTools.replace(curSong, " ", "-").toLowerCase()) {
					case 'senpai':
						schoolIntro(doof);
					case 'roses':
	...
	function iglesiatextbox(?dialogueBox:DialogueBox):Void {
		add(dialogueBox);
	}
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (player) {
				case 1:
					babyArrow.x += FlxG.width - 60 - (Note.swagWidth * 4);
				default:
					babyArrow.x += 60;
			}
			...
			trace("Arrow " + (i + 1) + " - x: " + babyArrow.x + ", y: " + babyArrow.y);
		}
	}
	...
	function afterGameplay():Void {
		if (dialogueEnd != null && isStoryMode) {
			...
			dbox.finishThing = afterStorySong;
			dbox.cameras = [camHUD];
			add(dbox);
			dbox.visible = true;
		} else {
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		if (daRating != 'shit' || daRating != 'bad')
		{
			...
			if (!PlayStateChangeables.botPlay) { //No mostrar combo en botplay
				var daLoop:Int = 0;
